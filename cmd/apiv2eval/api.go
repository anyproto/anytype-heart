...
type docBlock struct {
	...
	Views   []docView     `json:"views"`
}

// docView is one dataview view, in the shape a read SERVES it — captured
// from a live collection after a real update_view, not from the schema:
// sorts carry `direction`, filters are expanded server-side out of the
// compact string into {property, condition, value}, and a column that is
// visible carries NO hidden key at all rather than hidden:false.
type docView struct {
	Id      string          `json:"id"`
	Name    string          `json:"name"`
	Sorts   []docViewSort   `json:"sorts"`
	Filters []docViewFilter `json:"filters"`
	Columns []docViewColumn `json:"columns"`
}

type docViewSort struct {
	Property  string `json:"property"`
	Direction string `json:"direction"`
}

type docViewFilter struct {
	Property  string `json:"property"`
	Condition string `json:"condition"`
}

type docViewColumn struct {
	Property string `json:"property"`
...
// view returns the first view of the document's first dataview block. Every
// dataview task seeds a collection, which comes with exactly one of each.
func (d *document) view() (docView, bool) {
	for _, b := range d.Blocks {
		if b.Type == "dataview" && len(b.Views) > 0 {
