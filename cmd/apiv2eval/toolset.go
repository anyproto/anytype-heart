...
// collection ops remain out of scope; update_view is here because the
// dataview tasks measure it.
var opsArmOps = []string{
	...
	"set_cell",
	"set_properties",
	// the dataview tasks need this on BOTH surfaces, or the comparison
	// they exist to make — a structured change through one wrapper tool
	// versus through the raw op — cannot be drawn
	"update_view",
}
...
func newOpsToolset(ctx context.Context, client *apiClient, spaceId, objectId string) (*opsToolset, error) {
	...
	for _, op := range opsArmOps {
		...
		ts.specs = append(ts.specs, toolSpec{
			Name:        op,
			Description: fmt.Sprintf("PATCH op %q on the object being edited. Example: %s", op, example),
			Parameters:  schema,
		})
	}
	return ts, nil
