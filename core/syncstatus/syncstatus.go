...
var logSyncStatus = logger.NewNamed(syncStatusServiceName)

const syncStatusServiceName = "common.commonspace.syncstatus"

type UpdateReceiver interface {
	UpdateTree(ctx context.Context, treeId string, status SyncStatus) (err error)
	UpdateNodeStatus(status node.ConnectionStatus)
}
...
type StatusUpdater interface {
	...
	SetNodesStatus(senderId string, status node.ConnectionStatus)
	StateCounter() uint64
...
func NewNodeStatus() node.NodeStatus {
	return &syncStatusService{}
}
...
func (s *syncStatusService) SetNodesStatus(senderId string, status node.ConnectionStatus) {
...
func (s *syncStatusService) GetNodeStatus(senderId string) node.ConnectionStatus {
	if !s.isSenderResponsible(senderId) {
		return node.Online
	}
	s.Lock()
