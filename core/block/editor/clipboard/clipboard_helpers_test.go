...
func pasteAny(t *testing.T, sb *smarttest.SmartTest, id string, textRange model.Range, selectedBlockIds []string, blocks []*model.Block) []string {
	cb := newFixture(sb)
	req := &pb.RpcBlockPasteRequest{}
	if id != "" {
	...
	ids, _, _, _, err := cb.Paste(nil, req, "")
	require.NoError(t, err)
