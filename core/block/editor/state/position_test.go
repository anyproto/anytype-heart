...
func TestState_InsertTo(t *testing.T) {
	t.Run("default insert", func(t *testing.T) {
		...
		assert.Equal(t, []string{"child", "first", "second"}, r.Pick("root").Model().ChildrenIds)
		assert.True(t, r.Exists("first"))
		assert.True(t, r.Exists("second"))
	})
	t.Run("default insert - inner first", func(t *testing.T) {
		r := NewDoc("root", nil).(*State)
		r.Add(simple.New(&model.Block{Id: "root", ChildrenIds: []string{"child"}}))
		r.Add(simple.New(&model.Block{Id: "child"}))
		...
		s.Add(simple.New(&model.Block{Id: "first"}))
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("", model.Block_InnerFirst, "first", "second")
		require.NoError(t, err)
		...
		assert.Len(t, hist.Add, 2)
		assert.Len(t, hist.Change, 1)
		assert.Equal(t, []string{"first", "second", "child"}, r.Pick("root").Model().ChildrenIds)
		assert.True(t, r.Exists("first"))
		assert.True(t, r.Exists("second"))
	})
	t.Run("bottom", func(t *testing.T) {
		...
		s.Add(simple.New(&model.Block{Id: "first"}))
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("target", model.Block_Bottom, "first", "second")
		require.NoError(t, err)
	...
	t.Run("top", func(t *testing.T) {
		...
		s.Add(simple.New(&model.Block{Id: "first"}))
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("target", model.Block_Top, "first", "second")
		require.NoError(t, err)
	...
	t.Run("inner", func(t *testing.T) {
		...
		s.Add(simple.New(&model.Block{Id: "first"}))
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("target", model.Block_Inner, "first", "second")
		require.NoError(t, err)
	...
	t.Run("replace", func(t *testing.T) {
		...
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("target", model.Block_Replace, "first", "second")
		require.NoError(t, err)
	...
	moveFromSide := func(t *testing.T, pos model.BlockPosition) (r *State, c1, c2 simple.Block) {
		...
		s.Add(simple.New(&model.Block{Id: "first"}))
		s.Add(simple.New(&model.Block{Id: "second"}))
		err := s.InsertTo("target", pos, "first", "second")
		require.NoError(t, err)
	...
	t.Run("left to column", func(t *testing.T) {
		...
		err := s.InsertTo(c1.Model().Id, model.Block_Left, "third")
		require.NoError(t, err)
	...
	t.Run("left to column 2", func(t *testing.T) {
		...
		s := r.NewState()
		s.Add(simple.New(&model.Block{Id: "third"}))
		err := s.InsertTo(c2.Model().Id, model.Block_Left, "third")
		require.NoError(t, err)
	...
	t.Run("left to column 2", func(t *testing.T) {
		...
		s := r.NewState()
		s.Add(simple.New(&model.Block{Id: "third"}))
		err := s.InsertTo(c2.Model().Id, model.Block_Right, "third")
		require.NoError(t, err)
