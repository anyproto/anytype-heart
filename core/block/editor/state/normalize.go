...
func (s *State) normalizeChildren(b simple.Block) {
	...
	for _, cid := range m.ChildrenIds {
		if !s.Exists(cid) {
			s.cacheParent(m, slice.RemoveMut(m.ChildrenIds, cid))
			s.normalizeChildren(b)
			return
		}
	}
...
func (s *State) normalizeTree() (err error) {
	...
	if s.Pick(headerId) != nil && slice.FindPos(s.Pick(s.RootId()).Model().ChildrenIds, headerId) != 0 {
		...
		s.cacheParent(root, append([]string{headerId}, root.ChildrenIds...))
	}
	return nil
}
...
func (s *State) wrapChildrenToDiv(id string) (nextId string) {
	...
	div := s.newDiv()
	s.cacheParent(div.Model(), parent.ChildrenIds)
	s.Add(div)
	s.cacheParent(parent, []string{div.Model().Id})
	return parent.Id
}
...
func (s *State) divBalance(d1, d2 *model.Block) (overflow bool) {
	...
	s.cacheParent(d2, d1.ChildrenIds[div:])
	s.cacheParentUntilSame(d1, d1ChildrenIds, true)
