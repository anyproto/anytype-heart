...
func (oc *ObjectCreator) Create(dataObject *DataObject, sn *common.Snapshot) (*types.Struct, string, error) {
	...
	st.SetLocalDetail(bundle.RelationKeyLastModifiedDate.String(), pbtypes.Int64(snapshot.Details.GetInt64OrDefault(bundle.RelationKeyLastModifiedDate, 0)))
...
func (oc *ObjectCreator) setSpaceDashboardID(spaceID string, st *state.State) {
	...
	spaceDashBoardID := st.CombinedDetails().GetStringOrDefault(bundle.RelationKeySpaceDashboardId, "")
	if spaceDashBoardID != "" {
	...
	spaceName := st.CombinedDetails().GetStringOrDefault(bundle.RelationKeyName, "")
	if spaceName != "" {
		details = append(details, &model.Detail{
