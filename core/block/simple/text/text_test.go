...
func TestText_normalizeMarks_emojiNotMerged(t *testing.T) {
	t.Run("adjacent identical emojis are not merged", func(t *testing.T) {
		...
		// then
		require.Len(t, b.content.Marks.Marks, 2)
		assert.Equal(t, model.Range{From: 0, To: 1}, *b.content.Marks.Marks[0].Range)
		assert.Equal(t, model.Range{From: 1, To: 2}, *b.content.Marks.Marks[1].Range)
	})
	...
	t.Run("different emojis are not merged", func(t *testing.T) {
		...
		// then
		require.Len(t, b.content.Marks.Marks, 2)
		assert.Equal(t, "😀", b.content.Marks.Marks[0].Param)
		assert.Equal(t, "🎉", b.content.Marks.Marks[1].Param)
	})
	...
	t.Run("three identical emojis are all preserved", func(t *testing.T) {
		...
		// then
		require.Len(t, b.content.Marks.Marks, 3)
		assert.Equal(t, model.Range{From: 0, To: 1}, *b.content.Marks.Marks[0].Range)
		assert.Equal(t, model.Range{From: 1, To: 2}, *b.content.Marks.Marks[1].Range)
		assert.Equal(t, model.Range{From: 2, To: 3}, *b.content.Marks.Marks[2].Range)
	})
	...
	t.Run("emojis preserved while bold marks still merge", func(t *testing.T) {
		...
		boldCount := 0
		emojiCount := 0
		for _, m := range b.content.Marks.Marks {
