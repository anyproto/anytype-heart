...
- Containers holding the same kind of thing SHOULD share one type — several task trackers are all tasks. Each container stays its own list either way, so sharing a type costs nothing and gives one consistent shape instead of near-duplicate types. Give two containers different types only when their properties describe genuinely different things.
