...
// (AnyBlock v2 §2c) from a live space, wiring store, cache and writer around the
// shared composition (pkg/lib/anyblockjson/compose) on top of the extracted
// collection seam (core/block/export/collect).
//
// The pipeline is collect → plan → emit → finish: collection returns the complete doc set before
// anything is written; the plan fixes every path single-threaded from
// details alone (a pure per-id function with no collision machinery);
...
// scope).
//
// The RPC surface is model.Export_AnyBlockV2 (design Q6, settled as its
...
func (e *Exporter) ExportCollected(ctx context.Context, req Request, docs collect.Docs, wr Writer) (res Result, err error) {
	...
	index, properties, stats, err := composer.Finish()
	if err != nil {
	...
	if len(stats.OrphanUsedKeys) > 0 {
		log.Warnf("export %s: %d referenced propert(ies) nothing can define, so the dictionary states no format for them: %s",
			req.SpaceId, len(stats.OrphanUsedKeys), strings.Join(stats.OrphanUsedKeys, ", "))
	}
	if len(stats.RefusedOptions) > 0 {
		log.Errorf("export %s: vocabular(ies) the property dictionary cannot state, dropped so the rest of the bundle survives: %s",
			req.SpaceId, strings.Join(stats.RefusedOptions, "; "))
	}
	if properties != nil {
