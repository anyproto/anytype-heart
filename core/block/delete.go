...
func (s *Service) deleteRelationOptions(spaceId string, relationKey string) error {
	relationOptions, _, err := s.objectStore.SpaceId(spaceId).QueryObjectIDs(database.Query{
...
func (s *Service) OnDelete(id domain.FullID, workspaceRemove func() error) error {
	...
	if err := s.objectStore.SpaceId(id.SpaceID).DeleteObject(id.ObjectID); err != nil {
