...
// case-insensitive and Unicode-lenient. Known narrowing, recorded (§4.5):
// the EBNF's identifier is Unicode (UAX #31 — `Дата_выполнения` parses),
// while `key` below stays ASCII — GBNF character classes cannot state the
// letter/mark property classes, and an approximating byte range would admit
// spellings the parser rejects, the false-positive class the leaf split
// below exists to prevent. A grammar-constrained decoder therefore cannot
// emit a non-Latin property name; everything it CAN emit still parses,
// which is the tripwire gbnf_accept_test pins. The leaf splits on the condition's
