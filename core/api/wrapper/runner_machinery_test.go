...
func TestIdempotencyIdentity(t *testing.T) {
	...
	t.Run("a second ambiguity rewrite keeps the ORIGINAL pre-rewrite identity", func(t *testing.T) {
		...
		fx.stub(patch, 400, ambiguous)
		fx.stub(getPath, 200,
			`{"formatVersion":"2.0","type":"task","blocks":[{"id":"part-a-section-intro-e0001","type":"paragraph","text":"a"},{"id":"section-body-f0002","type":"paragraph","text":"b"}]}`)
		fx.stub(patch, 200, editOKBody)
...
func TestPrepareValuesIsOrderDeterministic(t *testing.T) {
	...
	fx := newFixture(t)
	idx := newPropertyIndex([]v2model.PropertyRow{
		{Key: "moodLevel", Format: "text"}, {Key: "mood_level", Format: "text"},
		{Key: "dueDate", Format: "date"}, {Key: "due_date", Format: "date"},
	})
