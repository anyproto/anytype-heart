...
// §8.26).
const testTableDoc = `{"formatVersion":"2.0","type":"page","properties":{"name":"Grid"},"blocks":[` +
...
func TestDescribeResilience(t *testing.T) {
	...
	t.Run("a failed option listing is marked, not silently optionless", func(t *testing.T) {
		...
		fx.stub("GET /v2/spaces/space1/types/task", 200,
			`{"formatVersion":"2.0","kind":"object_type","properties":{"name":"Task"},"type_settings":{"api_key":"task","property_definitions":[{"property":"status","name":"Status","format":"select"}]}}`)
...
func TestPropertyIndexFetchedOnce(t *testing.T) {
	...
	assert.Empty(t, sent[0].Query.Get("keys"),
		"?keys=name is a DOCUMENT read parameter — the row listings keep serving api keys plus names")
}
