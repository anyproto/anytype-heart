...
func (r *Runner) resolveObject(session *Session, ref, space string) (string, string, error) {
	...
	// to say where an object lives. It is deliberately NOT written back to
	// the session: handles resolve through Space, so moving it here would
	// re-point numbers the last find assigned somewhere else.
	if space != "" {
		return space, ref, nil
	}
	if session.Space == "" {
...
// hex). Block refs pass through verbatim; the server resolves a ref by exact
// id or unique suffix.
...
// patchOps sends a single-op PATCH — one tool call, one intent — with the
// idempotency machinery and the §7.4 ambiguity retry: on 400
// ambiguous_input the runner re-reads the object, rewrites the named block
// refs to the served id they uniquely tail-match, and retries once.
func (r *Runner) patchOps(ctx context.Context, session *Session, spaceId, objectId string, op map[string]any, refFields []string) (*v2model.EditResult, error) {
	return r.patchOpsBatch(ctx, session, spaceId, objectId, []map[string]any{op}, refFields)
}

// patchOpsBatch sends one PATCH carrying ops as ONE atomic batch — still
// one tool call, one intent: delete_block's multi-reference form is N
// spellings of the same act, and the server applies the whole batch to a
// child state committed with a single Apply, so an op that fails anywhere
// leaves the document untouched (v2service.applyPatchOps). refFields name
// the block-reference fields of EVERY op (each op is checked for each
// field). Machinery is patchOps': idempotency key over the resolved body,
// and the §7.4 ambiguity retry across all ops' refs.
func (r *Runner) patchOpsBatch(ctx context.Context, session *Session, spaceId, objectId string, ops []map[string]any, refFields []string) (*v2model.EditResult, error) {
	...
	body := map[string]any{"ops": opsAny}
	...
	if lw := session.LastWrite; lw != nil && len(lw.Rewrites) > 0 &&
		...
		for key, resolved := range lw.Rewrites {
			if i, field, ok := parseRewriteKey(key); ok && i < len(ops) {
				ops[i][field] = resolved // ops back body, so the hash below sees this
			}
		}
	}
	...
	rewrites, retryErr := r.retryAmbiguous(ctx, spaceId, objectId, ops, refFields)
	...
	if lw := session.LastWrite; lw != nil && lw.Key == key {
		...
		for key, resolved := range rewrites {
			lw.Rewrites[key] = resolved
		}
...
// retryAmbiguous re-reads the object and rewrites the ops' ref fields to
// the served id they now uniquely tail-match, returning the rewrites it
// made keyed by rewriteKey. The pool is the re-read document's own served
// ids (servedLocalIds) — never a session-retained map, which could be stale
// against the current document. This self-heals exactly one case: the
...
// meant — and the server's error surfaces untouched.
func (r *Runner) retryAmbiguous(ctx context.Context, spaceId, objectId string, ops []map[string]any, refFields []string) (map[string]string, error) {
	doc, err := r.client.raw(ctx, apiRequest{
		method: "GET",
		path:   "/v2/spaces/" + seg(spaceId) + "/objects/" + seg(objectId),
		// the name vocabulary, like every wrapper document read (D5) — the
		// local ids read here are spelling-independent, but one read rule
		// beats two
	...
	for i, op := range ops {
		for _, field := range refFields {
			...
			if count == 1 && match != ref {
				...
				rewrites[rewriteKey(i, field)] = match
			}
		}
	}
	return rewrites, nil
...
// rewriteKey names one rewritten op field in LastWrite.Rewrites. Op 0 keeps
// the bare field name — the format every single-op record has always used,
// so a session file written before batching replays unchanged — and later
// ops carry their index as "i.field".
func rewriteKey(i int, field string) string {
	if i == 0 {
		return field
	}
...
// parseRewriteKey inverts rewriteKey.
func parseRewriteKey(key string) (int, string, bool) {
	dot := strings.IndexByte(key, '.')
	if dot < 0 {
		return 0, key, true
	}
	i, err := strconv.Atoi(key[:dot])
	if err != nil || i < 0 || key[dot+1:] == "" {
		return 0, "", false
	}
	return i, key[dot+1:], true
}
...
// prefix (every tool but delete_block sends a single op; delete_block's
// multi-reference batch re-spells its ops[i] paths itself —
// deleteBatchError). Without this, a model that follows the
...
var opsVocab = []struct{ from, to string }{
	...
	{"ops[0].id", "block"},
	// update_view's channels: the op speaks set.filter/set.filters and
	// set.sorts, the tool's slots are `filter` and `sort` — a filter parse
	// refusal or an unknown-sort-key refusal must name the argument the
	// model can actually resend
	{"ops[0].set.filters", "filter"},
	{"ops[0].set.filter", "filter"},
	{"ops[0].set.sorts", "sort"},
	{"ops[0].", ""},
	...
	// the view ops' targeting refusals name "the op's block/view field"
	// (viewops.go resolveDataviewBlock / resolveViewIndex); the tool's slots
	// carry the same names, minus the op framing
	{"name one with the op's block field", "name one with block"},
	{"name one with the op's view field", "name one with view"},
}
