...
// v2_space_handler_test.go pins the Phase-7 space HTTP layer: the dry-run
...
func TestGetSpaceV2Handler(t *testing.T) {
	t.Run("returns the row from the space view", func(t *testing.T) {
		...
		var got apimodel.V2Space
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
		assert.Equal(t, apimodel.V2Space{Id: "space1", Name: "Work", Description: "The wiki"}, got)
	})
...
func TestUpdateSpaceV2Handler(t *testing.T) {
	t.Run("patches and returns the merged row", func(t *testing.T) {
		...
		var got apimodel.V2Space
		require.NoError(t, json.Unmarshal(w.Body.Bytes(), &got))
		assert.Equal(t, apimodel.V2Space{Id: "space1", Name: "Renamed", Description: "The wiki"}, got)
	})
