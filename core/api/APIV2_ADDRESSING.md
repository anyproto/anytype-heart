...
rewritten away). Historical companion to the AnyBlock specification; it fed
the format revision that closed the former §15.3 gap. Every claim about current behaviour is
...
1. **Lossless round trip** — backup, export/import, migration
   ...
   `any-block/format/v2/conformance/ANOMALIES.md`). Needs identity that survives renames and duplicates.
2. **API v2 and agents** — an LLM reads and writes the format. Needs
   ...
   ~24 tokens and models mutate opaque ids in flight. A 24-hex property key
   is unusable by a small model.
...
production data has them — `any-block/format/v2/conformance/ANOMALIES.md` #6, 7 objects on `tag`).
...
(the AnyBlock conformance anomaly #6 loss class closes), the unnamed option survives.
...
- **Backup/restore:** lossless including twins, unnamed entities and
  ...
  (AnyBlock conformance anomaly #6) closes.
...
### 7.3 AnyBlock specification changes
...
op-default); the AnyBlock specification takes §7.3. The one store-touching item is the §7.5
