...
Status: **built** · implementation record · 2026-08-14.

Covers the implemented plan item 3.3
(`DELETE /v2/spaces/{space_id}/objects/{object_id}`; APIV2.md §8.42,
APIV2_PLAN.md Wave 3) together with the requirement that makes it safe:
...
Two documents border this work. This spec **consumes the first and supersedes
one paragraph of the second**:
...
rule and derived-object guard. Its additional consistency rules are not
designed here.
...
The provenance foundation (§11.1–11.4) and API surface (§11.5–11.8) are
built together under GO-7383. The former lives in the core change pipeline;
the latter lives under `core/api`. Keeping both in one release is a contract:
the route without the immutable record would fail closed for every object.
