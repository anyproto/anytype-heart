...
var v2OpSchemas = map[string]v2SchemaKind{
	...
	"insertBlocks": {
		...
		schema: opSchema(`"op","blocks"`,
			...
			`"blocks":{"type":"array","minItems":1,"maxItems":256,"items":{"$ref":"#/$defs/block"},"description":"exactly one of after/before/inside targets the run; indent 0 = the insertion level"}`),
		example: `{"ops":[{"op":"insertBlocks","after":"b3","blocks":[{"type":"checkbox","text":"todo"}]}]}`,
	},
	"moveBlock": {
		endpoint: v2OpsEndpoint,
		schema: opSchema(`"op","id"`,
			`"op":{"const":"moveBlock"}`,
			`"id":{"$ref":"#/$defs/blockRef","description":"the block to move — its whole subtree moves with it"}`,
