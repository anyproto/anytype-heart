...
// v2_validate.go implements POST /v2/validate: structural + format-semantic
...
func (s *V2Service) ValidateDocument(data []byte) apimodel.V2ValidateResponse {
	resp := apimodel.V2ValidateResponse{Issues: []apimodel.V2Issue{}, Warnings: []apimodel.V2Issue{}}
	...
	if !errors.As(err, &validationErr) {
		resp.Issues = append(resp.Issues, apimodel.V2Issue{Message: fmt.Sprintf("validate document: %v", err)})
		return resp
	}
	for _, issue := range validationErr.Issues {
		resp.Issues = append(resp.Issues, apimodel.V2Issue{Path: issue.Path, Message: issue.Message})
	}
