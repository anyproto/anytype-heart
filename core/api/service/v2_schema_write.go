...
// v2_schema_write.go implements the Phase-2 schema write surface (APIV2.md
...
func (s *V2Service) CreateType(ctx context.Context, spaceId string, body []byte, dryRun bool) (*apimodel.V2CreateResult, error) {
	...
	if raw, ok := fields["kind"]; ok {
		...
		if err := json.Unmarshal(raw, &kind); err != nil || kind != "objectType" {
			return nil, apimodel.V2ValidationFailed("not a type document",
				apimodel.V2Issue{Path: "/kind", Message: "POST types accepts kind \"objectType\" documents only"})
		}
	...
	if _, ok := fields["blocks"]; ok {
		...
		return nil, apimodel.V2ValidationFailed("type blocks are not supported on create",
			apimodel.V2Issue{Path: "/blocks", Message: "omit blocks — the editor generates the type's default views", Hint: "customize views in the app after creating the type"})
	}
	...
	if envelope.Key != "" {
		if bundle.HasObjectTypeByKey(domain.TypeKey(envelope.Key)) {
			return nil, apimodel.V2ValidationFailed("type key is reserved",
				apimodel.V2Issue{Path: "/key", Message: fmt.Sprintf("%q is a bundled type — it already exists", envelope.Key)})
		}
		if _, exists := s.typeIdInSpace(spaceId, envelope.Key); exists {
			return nil, apimodel.V2ValidationFailed("type key already exists",
				apimodel.V2Issue{Path: "/key", Message: fmt.Sprintf("type %q already exists in space %q", envelope.Key, spaceId), Hint: fmt.Sprintf("update it with PATCH /v2/spaces/%s/types/%s", spaceId, envelope.Key)})
		}
	}
	...
	result := &apimodel.V2CreateResult{Key: envelope.Key, Created: resolvers.created()}
	if dryRun {
		result.DryRun = true
		return result, nil
	}
...
func typeDetailsFromSnapshot(snapshot *model.SmartBlockSnapshotBase, key string) (*types.Struct, error) {
	...
	if key != "" {
		...
		if err != nil {
			return nil, apimodel.V2ValidationFailed("invalid type key",
				apimodel.V2Issue{Path: "/key", Message: err.Error()})
		}
	...
	if v, ok := details.Fields[bundle.RelationKeyRecommendedLayout.String()]; ok {
		if name := v.GetStringValue(); name != "" {
			...
			if !ok {
				return nil, apimodel.V2ValidationFailed("unknown layout name",
					apimodel.V2Issue{Path: "/properties/recommendedLayout", Message: fmt.Sprintf("unknown layout %q", name), Hint: "common layouts: basic, todo, note, profile"})
			}
...
func (s *V2Service) UpdateType(ctx context.Context, spaceId, typeKey string, body []byte, dryRun bool) (*apimodel.V2CreateResult, error) {
	if err := s.ensureSpace(spaceId); err != nil {
		return nil, err
	}
	...
	if err := decoder.Decode(&patch); err != nil {
		return nil, apimodel.V2ValidationFailed("invalid type patch",
			apimodel.V2Issue{Message: err.Error(), Hint: "the patch accepts properties and typeProperties"})
	}

	var detailUpdates []*model.Detail
	for _, key := range sortedKeys(patch.Properties) {
		if !updatableTypeDetailKeys[key] {
			return nil, apimodel.V2ValidationFailed("property not updatable on a type",
				apimodel.V2Issue{Path: "/properties/" + key, Message: fmt.Sprintf("cannot update %q", key), Hint: "updatable: name, description, iconEmoji, recommendedLayout"})
		}
	...
	result := &apimodel.V2CreateResult{Id: typeId, Key: typeKey, Created: resolvers.created()}
	if dryRun {
		result.DryRun = true
		return result, nil
	}
...
func typeDetailValue(key string, raw json.RawMessage) (*types.Value, error) {
	if key == "recommendedLayout" {
		...
		return nil, apimodel.V2ValidationFailed("invalid recommendedLayout",
			apimodel.V2Issue{Path: "/properties/recommendedLayout", Message: "expected a layout name or number"})
	}
	var str string
	if err := json.Unmarshal(raw, &str); err != nil {
		return nil, apimodel.V2ValidationFailed("invalid property value",
			apimodel.V2Issue{Path: "/properties/" + key, Message: "expected a string"})
	}
...
func (s *V2Service) DeleteType(ctx context.Context, spaceId, typeKey string, dryRun bool) (*apimodel.V2CreateResult, error) {
	if err := s.ensureSpace(spaceId); err != nil {
		return nil, err
	}
	typeId, ok := s.typeIdInSpace(spaceId, typeKey)
	if !ok {
		return nil, apimodel.V2NotFound(fmt.Sprintf("type %q not found in space %q", typeKey, spaceId))
	}
	result := &apimodel.V2CreateResult{Id: typeId, Key: typeKey}
	if dryRun {
		result.DryRun = true
		return result, nil
	}
...
func (s *V2Service) CreateProperty(ctx context.Context, spaceId string, req apimodel.V2CreatePropertyRequest, dryRun bool) (*apimodel.V2CreateResult, error) {
	if err := s.ensureSpace(spaceId); err != nil {
		return nil, err
	}
	if req.Name == "" {
		return nil, apimodel.V2ValidationFailed("name is required",
			apimodel.V2Issue{Path: "/name", Message: "a property needs a display name"})
	}
	...
	if len(req.Options) > 0 && !isSelect {
		return nil, apimodel.V2ValidationFailed("options need a select format",
			apimodel.V2Issue{Path: "/options", Message: fmt.Sprintf("options apply to select and multiSelect properties, not %q", req.Format)})
	}
	if req.Key != "" {
		if s.propertyKeyExists(spaceId, req.Key) {
			return nil, apimodel.V2ValidationFailed("property key already exists",
				apimodel.V2Issue{Path: "/key", Message: fmt.Sprintf("property %q already exists", req.Key), Hint: fmt.Sprintf("update it with PATCH /v2/spaces/%s/properties/%s", spaceId, req.Key)})
		}
	}

	result := &apimodel.V2CreateResult{Key: req.Key}
	if dryRun {
		result.DryRun = true
		result.Created = &apimodel.V2SideEffects{
			Properties: []apimodel.V2PropertyRow{{Key: req.Key, Name: req.Name, Format: req.Format}},
		}
		for _, opt := range req.Options {
			result.Created.Options = append(result.Created.Options, apimodel.V2CreatedOption{Property: req.Key, Name: opt.Name})
		}
		return result, nil
	}
	...
	for _, opt := range req.Options {
		...
		result.Created.Options = append(result.Created.Options, apimodel.V2CreatedOption{Property: resp.Key, Name: opt.Name})
	}
	return result, nil
}

// UpdateProperty implements PATCH /v2/spaces/{spaceId}/properties/{key}.
func (s *V2Service) UpdateProperty(ctx context.Context, spaceId, propertyKey string, req apimodel.V2UpdatePropertyRequest, dryRun bool) (*apimodel.V2CreateResult, error) {
	if err := s.ensureSpace(spaceId); err != nil {
		return nil, err
	}
	rel, err := s.store.SpaceIndex(spaceId).GetRelationByKey(propertyKey)
	if err != nil || rel == nil {
		return nil, apimodel.V2NotFound(fmt.Sprintf("property %q not found in space %q — list available keys with GET /v2/spaces/%s/properties", propertyKey, spaceId, spaceId))
	}
	if bundled, err := bundle.PickRelation(domain.RelationKey(propertyKey)); err == nil && bundled.ReadOnly {
		return nil, apimodel.V2ValidationFailed("property is read-only",
			apimodel.V2Issue{Path: "/name", Message: fmt.Sprintf("bundled property %q cannot be updated", propertyKey)})
	}

	result := &apimodel.V2CreateResult{Id: rel.Id, Key: propertyKey}
	if req.Name == nil {
		return nil, apimodel.V2ValidationFailed("nothing to update",
			apimodel.V2Issue{Message: "the patch accepts name"})
	}
	if dryRun {
		result.DryRun = true
		return result, nil
...
// (archive).
func (s *V2Service) DeleteProperty(ctx context.Context, spaceId, propertyKey string, dryRun bool) (*apimodel.V2CreateResult, error) {
	if err := s.ensureSpace(spaceId); err != nil {
		return nil, err
	}
	rel, err := s.store.SpaceIndex(spaceId).GetRelationByKey(propertyKey)
	if err != nil || rel == nil {
		return nil, apimodel.V2NotFound(fmt.Sprintf("property %q not found in space %q", propertyKey, spaceId))
	}
	result := &apimodel.V2CreateResult{Id: rel.Id, Key: propertyKey}
	if dryRun {
		result.DryRun = true
