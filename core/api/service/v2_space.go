...
// SAME bound the `space` discovery kind advertises (v2_schemas.go): the
...
func validateSpaceField(path, value string) error {
	if length := utf8.RuneCountInString(value); length > maxSpaceFieldLength {
		return apimodel.V2ValidationFailed(strings.TrimPrefix(path, "/")+" is too long",
			apimodel.V2Issue{Path: path,
...
func (s *V2Service) CreateSpace(ctx context.Context, req apimodel.V2CreateSpaceRequest, dryRun bool) (*apimodel.V2Space, error) {
	...
	if resp.SpaceId == "" {
		...
		return nil, apimodel.NewV2Error(http.StatusInternalServerError, apimodel.V2CodeInternalError,
...
func v2SpaceRpcError(op string, code, badInputCode int32, description string) error {
	...
	case strings.Contains(description, "space not exists"),
		...
		return apimodel.V2NotFound(fmt.Sprintf("%s: %s — list live spaces with GET /v2/spaces", op, description))
