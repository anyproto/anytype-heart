...
### S6. `ensureSpace` lacks the liveness predicate — a deleted/left space stays fully addressable on ~20 routes **[R]**
...
and `SpaceIndex(spaceDead)` is minted as a side effect, the very thing the guard's doc comment
exists to prevent. Writes fail deep with 5xx instead of the clean 404. Fix: move the liveness
check into `ensureSpace` (hence `ensureSpaceWrite`) so the predicate is one choke point.
...
### S14. Edit-surface polish: empty-list residue and delete-then-recreate **[R]**

**(a)** `setProperties` remove of the last entry leaves `key: []` instead of unsetting
(`stateops.go:769` — unconditional `SetDetail`, verified): present-but-empty ≠ absent in the
§3 presence contract. Fix: `RemoveDetail` when `kept` is empty. **(b)** reusing a block id
