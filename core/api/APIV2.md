# Anytype Local API v2 — specification and implementation record

Status: **draft** · implementation record updated 2026-08-31
Depends on: AnyBlock JSON format version 2.0
(`github.com/anyproto/any-block/format/v2/SPEC.md`).

The sections below distinguish shipped behavior from the few remaining
build items. v1 (`/v1`) stays untouched for the deprecation window.
...
- **Eval harness** (`cmd/apiv2eval`): runs task-specific model loops against
  ...
  measures schema-emission behavior without a live API. The earlier generic
  corruption/backtranslation scorer was retired; `snapshotdiff` remains the
...
- **`outline=true` returns every block** as `{indent, id, type, text}`.
  Text is capped at 80 runes and receives a trailing ellipsis when
  truncated, so the outline stays a cheap survey while every block remains
  addressable for a follow-up `?block=` read or PATCH.
...
- `types/{type}/schema` **[build]**: the derived artifact (SPEC §2a
  ...
  store-backed option join stays an open §3 build item. Until it lands,
  the wrapper's shipped `describe` tool assembles the type and its live
  option lists itself. The route remains the highest-leverage accuracy
  improvement (§3.4).
- **`DELETE /objects` (archive) is built**: the registered route archives
  only objects created by the calling API key, enforced from immutable
  creation provenance. It supports idempotency and dry-run; see §8.42 for
  the complete fail-closed rule. The task-tool wrapper does not currently
  expose an object-archive verb.
...
row→column and column→row layout containment, bounds, id uniqueness). Any violation rejects the **whole
...
- **`set_properties` `add`/`remove` (v0.3.5)**: per-key list edits for
  list-shaped formats only (select, multi_select, objects, files — SPEC §3).
  `{"op":"set_properties","add":{"tags":["urgent"]},"remove":{"assignee":["bafy…"]}}`
  — `add` appends entries without duplicating existing ones; `remove`
  deletes matching entries, unsets the property when it consumes the final
  entry, and is a no-op when absent (never creates presence, never creates
  the option it names). Scalar-format keys are
  ...
  appending one tag to a 40-entry multi_select used to require read →
...
- Response: new `etag`, created-block id map **keyed by payload position**
  (`ops[3].blocks[0] → "b1a2…"`) for ids the server minted. A
  client-supplied payload id resolves an existing block and preserves that
  identity, so it is absent from `created_blocks`; `diff_stats` follows the
  same R14 receipt contract.
...
- **CLI verb naming**: `move-block`/`delete-block`, matching the tool
  ...
  deletion. The REST object-archive route is built, but the wrapper does
  not expose it; a future wrapper verb should be named `archive`.
...
snapshots are for creates, edits are ops), compact edit reads with an
explicit full-id export shape, diff_stats · flat AnyBlock as the primary content
...
- ~~**the D′1 escape decision** for `edit_text`/`replace_text` (§7.1)~~ —
  **BUILT**: `find` matches the served inline source; `replace` is literal
  prose applied in the parsed text-and-marks model.
...
The current `cmd/apiv2eval` harness uses task-specific final-state graders
and records outcomes, refusals, output tokens and turns per model tier. Its
small-tier wrapper is grammar-constrained. The generic
corruption/backtranslation metric described in early plans is not part of
the current harness.
...
- **B1 — edit-primitive steering** (tunes documentation; ships nothing —
  ...
  withheld from the prompt) · **full launch op set**. A planned
  whole-document corruption baseline left with PUT and the retired generic
  scorer; it was never a gate arm. Decision rule: whether the
...
1. As built: `/v2` ships **ungated** alongside v1 on the same localhost
   ...
   day one.
2. Phase 2, then 3a/3b — shipped. The Phase 1–2 R11 additions also ship:
   files, spaces and members, plus object archive (`DELETE /objects`;
   own-output-only as specified in §8.42).
...
- **Editing channel = anchor + deterministic server edit.** `edit_text`
  ...
  `replace_text` matches the block's §8 markup source, but replacement text
  is literal: the server parses a context-safe placeholder, then replaces it
  in the text-and-marks model, so a replacement cannot mint marks or break
  marks enclosing the match. A `find` confined to markup metadata (a tag
  attribute or link destination rather than rendered prose) is rejected
  path-addressed; use the relevant structured block operation instead. Dense
  `replace_all` output is size-checked before allocation against the block
  text's 1,048,576 UTF-16-unit limit. The
...
- **Reference channel = enumerated handles.** `find` returns `1,2,3`; both
  ...
  suffixes, and every write path resolves suffixes via `matchBlockRef`, so
  the wrapper passes them through unchanged. It resolves object handles and
...
other end), multi-op
batch (single tool call per intent — delete_block's comma-list is not the
exception but the rule restated: N references, one intent, one atomic
PATCH), the structured `filters` array (recursive, not
...
3. **Conveniences, placed** — scalar→array coercion is **already served**
   ...
   the wrapper resolves `@me` and relative dates while the REST value path
   stays literal. The wrapper also pre-validates option names for the small
   tier (§7.4); REST keeps its R9 create-missing behavior.
   If-Match/Idempotency-Key management stays wrapper-owned (the model
   authors none of these).

### 7.4 Wrapper implementation status

The Phase-5 wrapper work is built:
...
- **per-tool GBNF/CFG artifacts** — generated from the served tool schemas,
...
- ~~**the D′1 escape decision** for `edit_text`~~ — built by applying
  literal `replace` text in the parsed text-and-marks model.
...
same-key requests from the same credential never double-execute (the second
blocks then replays). Separate credentials never replay or conflict with one
another, even when they use the same key and request in the same space.
`POST /validate` uses the same credential namespace with an empty space axis;
it is the deliberate data-free-read extension to the mutation rule.
...
**Eval-harness status.** The separate Phase-0 scoring-primitives package was
retired. The current `cmd/apiv2eval` owns the task fixtures, model loop,
task-specific final-state graders, token/turn accounting and schema-emission
probe. It runs against a dedicated API space and the product's wrapper or
raw-op surfaces. The state-diff/text-multiset comparator remains in
`snapshotdiff` for format corpus round trips; the API harness does not expose
the proposed generic backtranslation score.
...
`properties`/`spaces`/`members` lists) + the initial eval plumbing.
Exit criterion: Phase 1 reads pass the fixture task set at parity-or-better
vs the v1 GET flow (fewer tokens for the same content). Phases 2–7 are out
of this increment.
...
**Create-vs-reject policy (R9), explicit.** Select/multi_select option
*names* → **create-missing** everywhere they appear as values (SPEC §3);
two live options with the same name make that bare name **ambiguous** and
always return `ambiguous_input` listing every matching label/color — store
order is never a tie-breaker, even with `create_missing_options=true`;
...
number. Export writes the canonical layout name. A type document carrying
`blocks` (its dataview) is **rejected explicitly** for now — the editor generates default
...
and structured `filters` form both ship and converge on one filter tree;
supplying both returns 400 `ambiguous_input` (C6). `views` is mutually
exclusive with top-level `filters`/`sorts`.

**Shortcut markdown.** `{type,name,properties,markdown}` parses markdown to
flat blocks and folds it into the synthesized snapshot before object
creation. The create and its blocks therefore use one change set; dry runs
validate the complete would-be document and do not write.
...
the orphaned subtree). replace_text → find on the block's document text,
parse a context-safe placeholder for ordinary inline text, then replace the
placeholder in the text+marks model (code/embed stay literal). set_cell → edit the cell on the
...
create surface in PATCH payloads is set_properties select/multi_select
...
(select/multi_select/objects/files); scalar-format keys are rejected
...
nothing. `remove` consuming the last entry unsets the key; `remove` of an
absent key stays absent. SPEC §3 presence semantics remain explicit:
`set: []` still means present-but-empty. Key
...
guard** pre-validates select/multi_select names in `create`/
...
forms), filter-string examples, and the caveats (literal replacement with
markup-source matching,
...
- **Status predicate, re-decided in the review hardening.** GET-one, the
  spaces LIST, global-search fan-out and every space-scoped route serve
  **live spaces only** — `isLiveSpaceView`, v1's two-axis predicate (local
  status Unknown/Ok AND account status Unknown/SpaceActive). `ensureSpace`
  applies it before opening a per-space index, so a retained view for a
  deleted, left or still-joining space cannot expose stale nested data. The
  original as-built served any space with a view; the review showed a
  deleted space's row is indistinguishable from a live one (the shape has no
  status slot), so an agent picking it would PATCH or write into a space that
  can never load. One intentional asymmetry remains: `ensureSpace`
  short-circuits the TECH space, so internal space-scoped calls may address
  it while GET-one never advertises it.
...
- **OpenAPI**: `make openapi` is green again and the `docs/v2` documents
  ...
  Cross-cutting response statuses are applied after swag generation from
  the router policy in `scripts/fix_openapi_v2.py`: every operation declares
  shared query-validation 400 plus authentication 401 and scope 403; all 22
  idempotent operations declare 409; all 21 write-limited operations declare
  the shared legacy-envelope 429; bounded assertion-linked bodies declare
  413; and all ten create operations declare their dry-run 200 beside 201.
  Reusable response components distinguish the shared legacy 401/403/429
  envelopes from v2's 400/409/413 error shape. A drift test parses both
  checked-in artifacts, compares their exact operation/status inventories
  and pins every router policy set so regeneration cannot silently drop the
  declarations again.
...
the generated OpenAPI documents now include it.
...
has regenerated the documents with all 14 ops named by the PATCH annotation.
...
via the ambiguity 400 and round-trip serving. This pass changed no OpenAPI
annotation shapes; the current generated documents include the later route
and schema changes recorded in §8.14 and subsequent sections.
...
`TestServedOpExampleIsAnInstanceOfItsOwnSchema` independently pins the same
relation. The obsolete whole-request-vs-op example switch was removed; the
probe now consumes the one served op-level example directly.
...
These experiment-only arms were removed after the decision; the names below
identify the recorded run and are not accepted by the current harness.
...
and the fix it proposes covers both. The Phase-9 proposal was not
implemented and is retired by §8.35's short-reference solution.
...
One free consequence, recorded: the C8 idempotency key includes
`c.Param("space_id")` (`middleware.go`), which the middleware has already
rewritten — so a retry by the same credential that spells the space
differently still hits the same cached entry, rather than replaying the
mutation under a second key. The credential id is the other namespace axis,
so a different credential neither replays nor conflicts with that entry.
...
OpenAPI was regenerated after Waves 0–1; these waves changed description
and example text rather than annotation shapes. The `?ids=`/pins work, the §7.4 write
...
The same over-reach hit the AnyBlock specification, where it made the document contradict
...
3. **The AnyBlock format is being swept to snake_case on its own branch.**
   ...
   stopped pointing anywhere. (Superseded in turn: the format later moved
   to raw display names as its one vocabulary, and the API pinned its own
   served bodies to the minted slug with `?keys=name` as the opt-out —
   the whole decision is APIV2_VOCABULARY.md.)
...
- `v2service.TestOpVocabularyIsSnakeCase` asserts every served op name is
  snake_case. Strict op decoding separately rejects names outside the closed
  served vocabulary.
...
(That "the grammar's vocabulary is the format's" clause is exactly what the
format's later move to raw display names DID strain: a display name with
spaces has no compact-filter spelling, and the GBNF's `key` production is
narrower still (ASCII). The resolution is recorded where it was made —
