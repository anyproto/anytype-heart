...
//	@description					Each streamed message event carries the chat state id as its event id. Send the last one back as `Last-Event-ID` to resume a dropped connection and receive only what you missed. When the gap cannot be covered from the messages still held, the stream sends `resync_required` before a fresh window, and whatever you hold outside that window is unverified.
