...
//	@description					Every error has one shape: {status, code, message, issues:[{path, message, hint}]}. All four members are always present, and `issues` is an empty array when the refusal has no path to name, so reading it never has to test for absence. Each issue is addressed by path and names the values that would have been accepted, so a failed call tells you how to repair it. Some refusals answer before this API's own handlers and use the older shared envelope instead: authentication, key scope, request origin, the shared write rate limit, an out-of-range page limit, and an unmatched route.
