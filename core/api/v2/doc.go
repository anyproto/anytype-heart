...
//	@description					Every mutation and the data-free `POST /validate` read accept an `Idempotency-Key` header. The same key with the same request replays the stored response instead of repeating the operation. Search is a read carried by POST and takes no key.
