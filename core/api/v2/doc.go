...
//	@description					One vocabulary throughout (C2): camelCase, property keys, option names, and `type` as a type key — never an id, never snake_case. Responses are compact JSON (C3); the default object read spells object ids in full inline and relabels block ids to short doc-local suffixes, and `?ids=full` returns the export shape (full block ids + the `refs` legend) instead (C4); list and search rows carry id, name, type and the properties you asked for, and never embed type objects (C5).
