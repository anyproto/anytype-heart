...
//	@description					A chat's messages can also be streamed: GET /v2/spaces/{space_id}/chats/{chat_id}/messages/stream opens a Server-Sent Events connection that begins with the last `limit` messages as `message_added` events and then delivers `message_added`, `message_updated`, `message_deleted` and `reactions_updated` as they happen. Idle connections are held open with comment lines, which clients ignore.
//	@description					Each streamed addition carries the chat state id as its event id. Send the last one back as `Last-Event-ID` to resume a dropped connection. Only additions replay: an edit, a deletion, or a reaction that happened while you were away does not restamp the state id, so your copy of that message stays as it was until you read the messages again. When the gap cannot be covered from the messages still held, the stream sends `resync_required` before a fresh window, and whatever you hold outside that window is unverified. The event id is meaningful only against the node that issued it.
