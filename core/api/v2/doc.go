...
//	@description					One vocabulary throughout (C2): camelCase, property keys, option names, and `type` as a type key — never an id, never snake_case. Responses are compact JSON (C3) with compact object refs plus a legend (C4); list and search rows carry id, name, type and the properties you asked for, and never embed type objects (C5).
