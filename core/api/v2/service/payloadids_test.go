...
const editMintedTableDoc = `{"formatVersion":"2.0","id":"obj1","type":"page","blocks":[` +
...
const editSharedViewIdDoc = `{"formatVersion":"2.0","id":"obj1","type":"page","properties":{"name":"Doc"},"blocks":[` +
...
const editTailCollisionDoc = `{"formatVersion":"2.0","id":"obj1","type":"page","blocks":[` +
...
func TestPayloadIdsResolveViewsInsideBareObjectCells(t *testing.T) {
	const fullViewId = "0000000000000000000a5487"
	storedBlock := map[string]any{
		"type": "table",
	...
	vocab := (&v2EditDoc{blocks: []map[string]any{storedBlock}}).localIds()
	assert.Contains(t, vocab, fullViewId, "the pre-op vocabulary includes a bare cell's nested view")
	...
	err := walkPayloadIdSlots(block, "ops[0].set", applier.resolveOrMintSlot(vocab))

	require.NoError(t, err)
	rows := block["rows"].([]any)
	cells := rows[0].(map[string]any)["cells"].([]any)
	views := cells[0].(map[string]any)["views"].([]any)
	assert.Equal(t, fullViewId, views[0].(map[string]any)["id"])

	cellValue := map[string]any{
		"type":  "dataview",
		"views": []any{map[string]any{"id": "a5487", "name": "All"}},
	}
	require.NoError(t, applier.resolveCellValueIds(vocab, cellValue, "ops[1].value"))
	cellViews := cellValue["views"].([]any)
	assert.Equal(t, fullViewId, cellViews[0].(map[string]any)["id"])
}

func TestWalkPayloadIdSlotsCellPaths(t *testing.T) {
	block := map[string]any{
		"type": "table",
		"rows": []any{map[string]any{
			"id": "row-one",
			"cells": []any{
				map[string]any{
					"type":  "dataview",
					"views": []any{map[string]any{"id": "bare-view"}},
				},
				[]any{
					map[string]any{
						"type":  "dataview",
						"views": []any{map[string]any{"id": "root-view"}},
					},
					map[string]any{
						"id":    "descendant",
						"type":  "dataview",
						"views": []any{map[string]any{"id": "descendant-view"}},
					},
				},
			},
		}},
	...
	var paths []string
	require.NoError(t, walkPayloadIdSlots(block, "ops[0].set", func(slot payloadIdSlot) error {
	...
	assert.ElementsMatch(t, []string{
		"ops[0].set",
		"ops[0].set.rows[0]",
		"ops[0].set.rows[0].cells[0].views[0]",
		"ops[0].set.rows[0].cells[1][0].views[0]",
		"ops[0].set.rows[0].cells[1][1]",
		"ops[0].set.rows[0].cells[1][1].views[0]",
	}, paths)
	assert.NotContains(t, paths, "ops[0].set.rows[0].cells[0]", "a bare cell has a derived root id")
	assert.NotContains(t, paths, "ops[0].set.rows[0].cells[1][0]", "array element zero has a derived root id")
}
