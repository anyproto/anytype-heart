...
func TestV2GetObject(t *testing.T) {
	...
	t.Run("outline returns every block with its text", func(t *testing.T) {
		// given
		fx := newV2Fixture(t)
		fx.readerMock.EXPECT().ReadObject(mock.Anything, testSpaceId, "obj1").Return(testObjectRead(), nil)
		want := []v2model.OutlineEntry{
			{Indent: 0, Id: "h1", Type: "heading_1", Text: "Section"},
			{Indent: 0, Id: "p1", Type: "paragraph", Text: "parent"},
			{Indent: 1, Id: "p2", Type: "paragraph", Text: "child"},
			{Indent: 0, Id: "p3", Type: "paragraph", Text: "sibling"},
		}
	...
	t.Run("outline truncates text at 80 runes, never mid-code-point", func(t *testing.T) {
		// given: multibyte text — a byte-counting cut would slice a code
		// point (80 bytes of "æ" is 40 characters) and serve mojibake
		long := strings.Repeat("æ", 100)
		edge := strings.Repeat("ü", 80)
		fx := newV2Fixture(t)
		fx.readerMock.EXPECT().ReadObject(mock.Anything, testSpaceId, "obj1").Return(editRead(t,
			`{"formatVersion":"2.0","id":"obj1","type":"page","blocks":[`+
				`{"id":"paraLong1","type":"paragraph","text":"`+long+`"},`+
				`{"id":"paraEdge1","type":"paragraph","text":"`+edge+`"}]}`), nil)
		want := []v2model.OutlineEntry{
			{Indent: 0, Id: "paraLong1", Type: "paragraph", Text: strings.Repeat("æ", 80) + "…"},
			{Indent: 0, Id: "paraEdge1", Type: "paragraph", Text: edge},
		}
		...
		// then
		require.NoError(t, err)
		raw, err := json.Marshal(decodeBody(t, body)["outline"])
		require.NoError(t, err)
		var got []v2model.OutlineEntry
		require.NoError(t, json.Unmarshal(raw, &got))
		assert.Equal(t, want, got)
	...
	t.Run("read annotates table columns with the header row's text", func(t *testing.T) {
		// given
		fx := newV2Fixture(t)
		fx.readerMock.EXPECT().ReadObject(mock.Anything, testSpaceId, "obj1").Return(editRead(t, editTableDoc), nil)
		want := []any{
			map[string]any{"id": "colA", "header": "Name"},
			map[string]any{"id": "colB", "header": "Status"},
		}
		...
		// then
		require.NoError(t, err)
		table := decodeBody(t, body)["blocks"].([]any)[0].(map[string]any)
		assert.Equal(t, want, table["columns"])
	})

	t.Run("a table without a header row serves plain column ids", func(t *testing.T) {
		// given: the first row is DATA — an invented header would be a lie
		fx := newV2Fixture(t)
		fx.readerMock.EXPECT().ReadObject(mock.Anything, testSpaceId, "obj1").Return(editRead(t,
			`{"formatVersion":"2.0","id":"obj1","type":"page","blocks":[{"id":"tblOne1","type":"table",`+
				`"columns":[{"id":"colA"},{"id":"colB"}],`+
				`"rows":[{"id":"rowA","cells":["Name","Status"]}]}]}`), nil)
		want := []any{
			map[string]any{"id": "colA"},
			map[string]any{"id": "colB"},
		}
...
func TestV2ListObjects(t *testing.T) {
	...
	t.Run("unknown requested field is a repairable 400", func(t *testing.T) {
		fx := newV2Fixture(t)
		fx.objectStore.AddObjects(t, testSpaceId, []objectstore.TestObject{{
			bundle.RelationKeyId:             domain.String("rel-list-fields-validation-probe"),
			bundle.RelationKeyRelationKey:    domain.String("list_fields_validation_probe"),
			bundle.RelationKeyName:           domain.String("List fields validation probe"),
			bundle.RelationKeyRelationFormat: domain.Int64(int64(model.RelationFormat_longtext)),
			bundle.RelationKeyResolvedLayout: domain.Int64(int64(model.ObjectType_relation)),
		}, {
			bundle.RelationKeyId:                               domain.String("list-fields-validation-object"),
			bundle.RelationKeyName:                             domain.String("List fields validation fixture"),
			bundle.RelationKeyType:                             domain.String("type-task"),
			bundle.RelationKeyResolvedLayout:                   domain.Int64(int64(model.ObjectType_basic)),
			bundle.RelationKeyLastModifiedDate:                 domain.Int64(100),
			domain.RelationKey("list_fields_validation_probe"): domain.String("value that must not silently disappear"),
		}})

		_, _, _, err := fx.ListObjects(context.Background(), testSpaceId,
			[]string{"list_fields_validation_porbe"}, 0, 25)
		...
		assert.Equal(t, "list-fields-validation-object", rows[0].Id)
		assert.Equal(t, map[string]any{
			"list_fields_validation_probe": "value that must not silently disappear",
		}, rows[0].Properties)
	})
