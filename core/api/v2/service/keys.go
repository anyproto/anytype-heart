...
// keys.go implements the space's live type/property key surface. One
// bounded details query per kind primes entries carrying the
...
// liveProperties lists the space's live properties in one bounded query
// (tens to low
...
// resolvePropertyInput applies the resolution chain for one inbound
// property term over a primed live set (entries are mandatory —
...
// (lowercase, `_`/`-` stripped — `dueDate` for `due_date`).
// (5) display names (exact NFC name over visible live
// entries and the bundled name tables, then the format's wider FoldKeyTerm
// fold, which also strips whitespace, so "Due date" and "due date" land).
// Hidden entries answer to their exact stored key only (step 1) and are
// invisible to the slug, fold and name steps — see propertyEntry.Hidden.
...
func (s *Service) resolvePropertyInput(input string, entries []propertyEntry) (propertyEntry, bool, []string) {
	...
	// 5: display names — an agent reading "Due date" from
	// a ?keys=name body must be able to use it on every channel. Names join
	// the chain LAST: a live stored key or slug always outranks a name (the
	// verbatim-first obligation — a name may never shadow an address), and
	// running after step 4 keeps every resolution that works today working
	// identically (FoldApiKey keeps spaces, so only spellings with
	// whitespace — or matching nothing at all — ever reach here). Nothing is
	// ever auto-created from this step.
	// 5a: exact NFC name over VISIBLE live entries (NFC on both sides — a
	// decomposed spelling names the same property; FoldKeyTerm's docs own
	// the why). Two visible holders of one name is the loud ambiguity;
	// hidden entries never answer by name (propertyEntry.Hidden).
	nfcInput := norm.NFC.String(input)
	var nameMatches []propertyEntry
	for _, entry := range entries {
		if !entry.Hidden && entry.Name != "" && norm.NFC.String(entry.Name) == nfcInput {
			nameMatches = append(nameMatches, entry)
		}
	}
	if len(nameMatches) > 1 {
		return propertyEntry{}, false, describePropertyEntries(nameMatches)
	}
	if len(nameMatches) == 1 {
		// the slug step's no-shadowing discipline, on names: a live visible
		// holder of a bundled NAME neither silently wins over the bundled
		// binding nor silently loses to it — the input is ambiguous, refused
		// with both holders named (wrong-and-silent becomes actionable)
		if shadowed, ok := shadowedBundledPropertyName(nfcInput, nameMatches[0].Key); ok {
			return propertyEntry{}, false, append(describePropertyEntries(nameMatches), shadowed)
		}
		return nameMatches[0], true, nil
	}
	// 5a-bundled: the exact bundled display name, nothing live claiming it —
	// the installed entry wins where present (a renamed installed bundled
	// property still answers to its bundled name WITH its Id, exactly as the
	// bundled slug does at step 3)
	if key, ok := anyblockjson.BundledPropertyKeyByName(nfcInput); ok {
		return bundledPropertyCandidate(key, entries), true, nil
	}
	// 5b: the format's fold (FoldKeyTerm — case, `_`/`-`, whitespace,
	// invisibles) over visible NAMES and the bundled name fold tables;
	// single candidate resolves, several refuse — exactly step 4's rule
	nameFold := anyblockjson.FoldKeyTerm(input)
	if nameFold == "" {
		return propertyEntry{}, false, nil
	}
	var nameCandidates []propertyEntry
	nameSeen := map[string]bool{}
	for _, entry := range entries {
		if entry.Hidden || entry.Name == "" {
			continue
		}
		if anyblockjson.FoldKeyTerm(entry.Name) == nameFold && !nameSeen[entry.Key] {
			nameSeen[entry.Key] = true
			nameCandidates = append(nameCandidates, entry)
		}
	}
	for _, key := range anyblockjson.BundledPropertyKeysByFold(input) {
		if nameSeen[key] {
			continue
		}
		nameSeen[key] = true
		nameCandidates = append(nameCandidates, bundledPropertyCandidate(key, entries))
	}
	if len(nameCandidates) == 1 {
		return nameCandidates[0], true, nil
	}
	if len(nameCandidates) > 1 {
		return propertyEntry{}, false, describePropertyEntries(nameCandidates)
	}
	return propertyEntry{}, false, nil
}
...
func (s *Service) resolveTypeInput(input string, entries []typeEntry) (typeEntry, bool, []string) {
	...
	for _, key := range anyblockjson.BundledTypeKeysByFold(input) {
		if nameSeen[key] {
			continue
		}
		nameSeen[key] = true
...
// live-defect shape: a UI property named "Due Date"
...
// What CAN be fixed without touching
...
// shadowedBundledPropertyName is shadowedBundledProperty on the NAME table
// (chain step 5a): only a name the bundled table itself binds to a DIFFERENT
// key shadows anything — a bundled property answering to its own name is not
// a shadow, and an ambiguous bundled name binds nobody (the table refuses it
// at build time), so it cannot shadow either.
func shadowedBundledPropertyName(name, matchedKey string) (string, bool) {
	key, ok := anyblockjson.BundledPropertyKeyByName(name)
	if !ok || key == matchedKey {
		return "", false
	}
	rel := bundle.MustGetRelation(domain.RelationKey(key))
	return fmt.Sprintf("the bundled %q (key %s)", rel.Name, key), true
}

// shadowedBundledTypeName is shadowedBundledPropertyName for types.
func shadowedBundledTypeName(name, matchedKey string) (string, bool) {
	key, ok := anyblockjson.BundledTypeKeyByName(name)
	if !ok || key == matchedKey {
		return "", false
	}
	return fmt.Sprintf("the bundled %q (key %s)", bundle.MustGetType(domain.TypeKey(key)).Name, key), true
}

// bundledPropertyCandidate answers a bundled key that step 5 resolved: the
// installed live entry when the space holds one (so address routes see its
// Id even under a live rename), the synthetic Id=="" entry otherwise —
// exactly the shape step 3's bundled arm returns.
func bundledPropertyCandidate(key string, entries []propertyEntry) propertyEntry {
	for _, entry := range entries {
		if entry.Key == key {
			return entry
		}
	...
	rel := bundle.MustGetRelation(domain.RelationKey(key))
	return propertyEntry{Key: key, Name: rel.Name, Format: rel.Format}
}

// bundledTypeCandidate is bundledPropertyCandidate for the type namespace.
func bundledTypeCandidate(key string, entries []typeEntry) typeEntry {
	for _, entry := range entries {
		if entry.Key == key {
			return entry
		}
	}
	return typeEntry{Key: key, Name: bundle.MustGetType(domain.TypeKey(key)).Name}
}
...
//     internal key), verified for every relation creation
...
// in this space from its derived identity and point-looks-up its
