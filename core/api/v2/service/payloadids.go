...
func walkPayloadIdSlots(block map[string]any, path string, fn slotVisitor) error {
	...
	return walkNestedPayloadIdSlots(block, path, fn)
}

// walkNestedPayloadIdSlots descends into the id-bearing fields of a block
// without visiting the block's own id. Table-cell blocks use derived ids, so
// their root slot is not caller-owned, but a bare-object dataview cell can
// still carry views whose ids must resolve through the document vocabulary.
func walkNestedPayloadIdSlots(block map[string]any, path string, fn slotVisitor) error {
...
func walkEntryIdSlots(entries []any, field, path string, fn slotVisitor) error {
	for j, e := range entries {
		...
		for k, cell := range cells {
			cellPath := fmt.Sprintf("%s.cells[%d]", entryPath, k)
			switch value := cell.(type) {
			case []any:
				if err := walkCellRunIdSlots(value, cellPath, fn); err != nil {
					return err
				}
			case map[string]any:
				// The cell block's own id is derived, but nested rows, columns,
				// and views carry ordinary payload identities.
				if err := walkNestedPayloadIdSlots(value, cellPath, fn); err != nil {
					return err
				}
			}
		}
...
func walkCellRunIdSlots(run []any, path string, fn slotVisitor) error {
	for i := 0; i < len(run); i++ {
		...
		if i == 0 {
			// The first element is the derived-id cell block. Its nested
			// identity-bearing fields are still caller-addressable.
			if err := walkNestedPayloadIdSlots(el, entryPath, fn); err != nil {
				return err
			}
			continue
		}
		if err := walkPayloadIdSlots(el, entryPath, fn); err != nil {
			return err
		}
	}
	return nil
...
func (a *v2StateApplier) resolveCellValueIds(vocab []string, value any, path string) error {
	...
	case []any:
		return walkCellRunIdSlots(cell, path, a.resolveOrMintSlot(vocab))
	case map[string]any:
		return walkNestedPayloadIdSlots(cell, path, a.resolveOrMintSlot(vocab))
	default:
		return nil
	}
}
