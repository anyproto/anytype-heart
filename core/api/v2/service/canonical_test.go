...
func TestCanonicalizationEquivalence(t *testing.T) {
	...
	resolvers := fx.newCreatingResolvers(context.Background(), testSpaceId, true, true)
...
func TestV2MintShadowingClosed(t *testing.T) {
	...
	t.Run("a camel respelling resolves to the legacy stored key, never a twin mint", func(t *testing.T) {
		...
		result, err := fx.CreateType(context.Background(), testSpaceId,
			[]byte(`{"kind":"object_type","properties":{"name":"Gizmo"},"type_settings":{"api_key":"gizmo","property_definitions":[{"property":"myKey","section":"featured"}]}}`), false, true)
	...
	t.Run("a spelling whose fold misses but whose slug collides is refused", func(t *testing.T) {
		// "My_Key!" folds to "mykey!" (the format's fold strips case and
		// separators but keeps the "!") so the chain misses — but its minted
		// slug my_key would shadow the legacy stored key. The union check on
		// the SLUG spelling refuses loudly. ("My Key" no longer exercises
		// this: FoldKeyTerm strips spaces, so it now RESOLVES to the legacy
		// relation like the camel respelling above — the raw-name format's
		// intended forgiveness.)
		fx := legacy(t)

		_, err := fx.CreateType(context.Background(), testSpaceId,
			[]byte(`{"kind":"object_type","properties":{"name":"Gizmo 2"},"type_settings":{"api_key":"gizmo2","property_definitions":[{"property":"My_Key!","name":"My_Key!","format":"text"}]}}`), false, true)
	...
	t.Run("two spellings of one key in one request are refused, not twin-minted", func(t *testing.T) {
		...
		_, err := fx.CreateType(context.Background(), testSpaceId,
			[]byte(`{"kind":"object_type","properties":{"name":"Warrantied"},"type_settings":{"api_key":"warrantied","property_definitions":[{"property":"warranty_until","name":"W","format":"date"},{"property":"warrantyUntil","name":"W2","format":"date"}]}}`), false, true)
...
func TestV2CanonicalizeDocumentKeysDeterministicError(t *testing.T) {
	...
	fx := slugSpaceFixture(t)
	body := []byte(`{"formatVersion":"2.0","properties":{"manual_property":"a","` + slugPropKey + `":"b","manualProperty":"c"}}`)
