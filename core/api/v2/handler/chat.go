...
//	@Description	`after` on its own walks forward, oldest first, continuing from `next_after`. Every other query, including `after` together with `before`, is anchored at the newest end of the range and walks backward from `next_before`. Both bounds are exclusive. Messages are always in ascending order. `message_count` is the number of messages the chat holds now, not the size of the range; `lifetime_message_count` counts every message ever posted, deleted ones included. `state.unread_messages` reflects the read watermark. Offset paging does not apply here, and `offset` is refused.
...
//	@Description	`up_to` is inclusive, and it and `last_state_id` both come from one messages read: the newest message's order, and the state's own id. An empty value for either would silently mark nothing, so it is refused. Messages that arrived after that state stay unread. The reactions scope marks every unread reaction and takes neither field. The receipt carries the chat's state after the move; `state.unread_messages` on any messages read reflects it too.
//	@Id				read_chat
//	@Tags			Chat
//	@Accept			json
//	@Produce		json
