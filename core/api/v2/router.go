...
type RouteDeps struct {
	...
	// CreateDisabled skips create routes when no creator
...
func RegisterRoutes(router *gin.Engine, deps RouteDeps) {
	...
	// `?keys=` (§4.2) is `?ids=`'s twin for KEY spellings: it picks the
	// vocabulary property and type keys are SERVED in — the minted api slug
	// (default) or the format's raw display names — on every AnyBlock body
	// this request is served. Accept always resolves both — keyshape.go.
	v2.Use(ensureKeysShape())
	...
	v2.GET("/auth/whoami",
		...
		v2handler.WhoamiHandler(deps.Service),
	)
	v2.POST("/validate",
		idempotencyMW,
		deps.AnalyticsEvent("V2Validate"),
		v2handler.ValidateHandler(deps.Service),
	)
	v2.GET("/spaces",
		deps.AnalyticsEvent("V2ListSpaces"),
		v2handler.ListSpacesHandler(deps.Service),
	)
	// The space read is a tech-space store query (no
	...
	v2.GET("/spaces/:space_id/types/:type/schema",
		...
		v2handler.GetTypeSchemaHandler(deps.Service),
	)
	...
	v2.GET("/spaces/:space_id/properties/:key/options",
		...
		v2handler.ListPropertyOptionsHandler(deps.Service),
	)
	// Search is a READ (POST only because the request
	...
	v2.POST("/search",
		...
		v2handler.GlobalSearchObjectsHandler(deps.Service),
	)
	...
	v2.GET("/spaces/:space_id/collections/:collection_id/objects",
		...
		v2handler.GetCollectionObjectsHandler(deps.Service),
	)
	v2.GET("/spaces/:space_id/collections/:collection_id/views",
		deps.AnalyticsEvent("V2GetCollectionViews"),
		v2handler.GetCollectionViewsHandler(deps.Service),
	)
	v2.GET("/schemas",
		deps.AnalyticsEvent("V2ListSchemas"),
		v2handler.SchemaIndexHandler(deps.Service),
	)
	v2.GET("/schemas/:kind",
		deps.AnalyticsEvent("V2GetSchema"),
		v2handler.SchemaKindHandler(deps.Service),
	)
	v2.GET("/schemas/ops/:op",
		deps.AnalyticsEvent("V2GetOpSchema"),
		v2handler.SchemaOpHandler(deps.Service),
	)
...
// registerChatRoutes registers the chat surface. Every mutation, including
// DELETE, runs behind the idempotency middleware:
...
func registerChatRoutes(v2 *gin.RouterGroup, deps RouteDeps, idempotencyMW gin.HandlerFunc) {
	v2.GET("/spaces/:space_id/chats",
		...
		v2handler.ListChatsHandler(deps.Service),
	)
	v2.POST("/spaces/:space_id/chats",
		deps.WriteRateLimit,
		idempotencyMW,
		deps.AnalyticsEvent("V2CreateChat"),
		v2handler.CreateChatHandler(deps.Service),
	)
...
// are for creates, edits are ops (§8.27),
...
func registerEditRoutes(v2 *gin.RouterGroup, deps RouteDeps, idempotencyMW gin.HandlerFunc) {
	...
	v2.PATCH("/spaces/:space_id/objects/:object_id",
		...
		v2handler.PatchObjectHandler(deps.Service),
	)
	// Object deletion archives only objects created by the calling key. The
	...
	v2.DELETE("/spaces/:space_id/objects/:object_id",
		...
		v2handler.DeleteObjectHandler(deps.Service),
	)
}

// registerCreateRoutes registers the create surface. EVERY mutation — POST,
// PATCH and DELETE — runs behind the C8 idempotency middleware (C8 covers all
...
func registerCreateRoutes(v2 *gin.RouterGroup, deps RouteDeps, idempotencyMW gin.HandlerFunc) {
	...
	v2.POST("/spaces/:space_id/collections",
		...
		v2handler.CreateCollectionHandler(deps.Service),
	)
