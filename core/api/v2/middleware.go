...
// the C8 idempotency store/middleware, C9 dry-run handling, and the C6
// error responder.
...
// idempotencyScope isolates a caller's retries from every other credential.
// Authentication installs ApiKeyInfo before the v2 route middleware runs.
// The empty fallback keeps the middleware usable by isolated unit routers.
func idempotencyScope(c *gin.Context) string {
	spaceId := c.Param("space_id")
	if info, ok := util.ApiKeyInfoFromCtx(c.Request.Context()); ok {
		return info.Id + "\x00" + spaceId
	}
	return spaceId
}
...
// every registered v2 DELETE: chat message, type, and property deletes
// alike, so C8 reads "every v2
...
// so the middleware covers it like POST.
//
...
func ensureIdempotency(store *idempotencyStore) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		stored, replay, owner := store.begin(scope, key)
		if replay {
		...
		defer func() { store.finish(scope, key, result) }()
