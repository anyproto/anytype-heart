...
func (a *Anytype) getFileIndexes(ctx context.Context, hash string) ([]lsmodel.FileIndex, error) {
...
func (a *Anytype) ImageByHash(ctx context.Context, hash string) (Image, error) {
	...
	var variantsByWidth = make(map[int]lsmodel.FileIndex, len(files))
	for _, f := range files {
