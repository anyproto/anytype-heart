...
func (mw *Middleware) UserDataImport(cctx context.Context, req *pb.RpcUserDataImportRequest) *pb.RpcUserDataImportResponse {
	...
	mw.setMnemonic(profile.Mnemonic)
	...
	if err := mw.stop(); err != nil {
		response(pb.RpcUserDataImportResponseError_UNKNOWN_ERROR, err)
	}
	...
	newAcc.Avatar = &model.AccountAvatar{Avatar: &model.AccountAvatarAvatarOfImage{Image: &model.BlockContentFile{Hash: profile.Avatar}}}
