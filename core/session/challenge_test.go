...
// TestSolveChallenge_MintsSessionWithChallengeScope guards against H1
// (docs/ApiKeyScopingResearch.md §2.2): the pairing session must carry the
// scope the challenge was created with, not the zero value (Limited). The
// JsonAPI case is the one that used to pass vacuously — the minted session
// silently held Limited gRPC privileges.
