...
func GetFileConfig(configPath string, cfg interface{}) error {
	...
	if !errors.Is(err, os.ErrNotExist) {
		...
		if err != nil {
			return fmt.Errorf("fail to get info about config file: %s", err)
		}

		if info.Size() > 0 {
			err = json.NewDecoder(cfgFile).Decode(cfg)
			if err != nil {
				return fmt.Errorf("invalid file config format: %s", err)
			}
		}
	}
...
func WriteJsonConfig(configPath string, cfg interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("failed to marshal new config: %s", err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to open cfg file for updating: %s", err)
	}
	defer cfgFile.Close()

	err = json.NewEncoder(cfgFile).Encode(newConfig)
	if err != nil {
		return fmt.Errorf("failed to save data to the config file: %s", err)
	}
