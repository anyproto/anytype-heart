...
func (a *Anytype) GetSmartblockLogs(smartblockID string) ([]SmartblockLog, error) {
	id, err := thread.Decode(smartblockID)
	if err != nil {
		return nil, err
	}
...
func (a *Anytype) GetSmartblockRecord(ctx context.Context, smartblockID string, recordID string) (*SmartblockRecord, error) {
	tid, err := thread.Decode(smartblockID)
	if err != nil {
		return nil, err
	}

	thrd, err := a.t.GetThread(ctx, tid)
	if err != nil {
		return nil, err
	}
	...
	rec, err := a.t.GetRecord(ctx, tid, rid)
	if err != nil {
		return nil, err
	}
