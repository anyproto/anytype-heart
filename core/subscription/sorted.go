...
func (s *service) newSortedSub(id string, spaceId string, keys []string, filter database.Filter, order database.Order, limit, offset int) *sortedSub {
	sub := &sortedSub{
		id:          id,
		spaceId:     spaceId,
		keys:        keys,
...
func (s *sortedSub) onChange(ctx *opCtx) {
	...
	if s.parent != nil {
		parentEntries, err := queryEntries(s.objectStore.SpaceId(s.spaceId), &database.Filters{FilterObj: s.parent.filter})
		if err != nil {
			panic(err)
		}
