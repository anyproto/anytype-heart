...
func TestPrepareSearchDocument_Success(t *testing.T) {
	...
	docs, _, err := indexerFx.prepareSearchDocs(context.Background(), objectstore.FullTextQueuedObject{ObjectId: "objectId1", SpaceId: "spaceId1"})
...
func TestPrepareSearchDocumentWithDetails(t *testing.T) {
	...
	docs, _, err := indexerFx.prepareSearchDocs(context.Background(), objectstore.FullTextQueuedObject{ObjectId: objectId, SpaceId: "spaceId1"})
	require.NoError(t, err)
