...
func TestReindexMarketplaceSpace(t *testing.T) {
	t.Run("reindex missing object", func(t *testing.T) {
		...
		checksum, err := indexerFx.store.GetChecksums("spaceId")
		assert.Nil(t, err)
		assert.NotNil(t, checksum)
		assert.Equal(t, ForceBundledObjectsReindexCounter, checksum.BundledObjects)
	})
	t.Run("reindex missing object - old counter in checksum", func(t *testing.T) {
		// given
		indexerFx := NewIndexerFixture(t)
		mockHasher := mock_indexer.NewMockHasher(t)
		mockHasher.EXPECT().Hash().Return("")
