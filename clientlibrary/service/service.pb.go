...
type ClientCommandsHandler interface {
	...
	DebugSystem(context.Context, *pb.RpcDebugSystemRequest) *pb.RpcDebugSystemResponse
...
func DebugSystem(b []byte) (resp []byte) {
	defer func() {
		if PanicHandler != nil {
			if r := recover(); r != nil {
				resp, _ = (&pb.RpcDebugSystemResponse{Error: &pb.RpcDebugSystemResponseError{Code: pb.RpcDebugSystemResponseError_UNKNOWN_ERROR, Description: "panic recovered"}}).Marshal()
				PanicHandler(r)
			}
		}
	}()

	in := new(pb.RpcDebugSystemRequest)
	if err := in.Unmarshal(b); err != nil {
		resp, _ = (&pb.RpcDebugSystemResponse{Error: &pb.RpcDebugSystemResponseError{Code: pb.RpcDebugSystemResponseError_BAD_INPUT, Description: err.Error()}}).Marshal()
		return resp
	}

	resp, _ = clientCommandsHandler.DebugSystem(context.Background(), in).Marshal()
	return resp
}
