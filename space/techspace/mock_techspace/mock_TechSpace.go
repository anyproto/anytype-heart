...
// DoUserDataObject provides a mock function with given fields: ctx, apply
func (_m *MockTechSpace) DoUserDataObject(ctx context.Context, apply func(userdataobject.UserDataObject) error) error {
	ret := _m.Called(ctx, apply)

	if len(ret) == 0 {
		panic("no return value specified for DoUserDataObject")
	}

	var r0 error
	if rf, ok := ret.Get(0).(func(context.Context, func(userdataobject.UserDataObject) error) error); ok {
		r0 = rf(ctx, apply)
	} else {
		r0 = ret.Error(0)
	}
...
// MockTechSpace_DoUserDataObject_Call is a *mock.Call that shadows Run/Return methods with type explicit version for method 'DoUserDataObject'
type MockTechSpace_DoUserDataObject_Call struct {
	*mock.Call
}

// DoUserDataObject is a helper method to define mock.On call
//   - ctx context.Context
//   - apply func(userdataobject.UserDataObject) error
func (_e *MockTechSpace_Expecter) DoUserDataObject(ctx interface{}, apply interface{}) *MockTechSpace_DoUserDataObject_Call {
	return &MockTechSpace_DoUserDataObject_Call{Call: _e.mock.On("DoUserDataObject", ctx, apply)}
}

func (_c *MockTechSpace_DoUserDataObject_Call) Run(run func(ctx context.Context, apply func(userdataobject.UserDataObject) error)) *MockTechSpace_DoUserDataObject_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(context.Context), args[1].(func(userdataobject.UserDataObject) error))
	})
	return _c
}

func (_c *MockTechSpace_DoUserDataObject_Call) Return(err error) *MockTechSpace_DoUserDataObject_Call {
	_c.Call.Return(err)
	return _c
}

func (_c *MockTechSpace_DoUserDataObject_Call) RunAndReturn(run func(context.Context, func(userdataobject.UserDataObject) error) error) *MockTechSpace_DoUserDataObject_Call {
	_c.Call.Return(run)
	return _c
}
