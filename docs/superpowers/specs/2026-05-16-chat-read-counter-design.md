...
5. **Incremental cursor.** Persist device-local `lastSeenAddSeq` per counter so each pass only evaluates `GetAfterAddSeq(lastSeenAddSeq)` instead of all messages. Optional optimization — `SetReadFlag` is idempotent so correctness does not depend on it; it bounds the work.
