...
func TestOptionsNamedReturnsEveryDuplicate(t *testing.T) {
	fx := newFixture(t)
	fx.index.AddObjects(t, []spaceindex.TestObject{
		{
			bundle.RelationKeyId:                  domain.String("opt-high-2"),
			bundle.RelationKeyRelationKey:         domain.String("priority"),
			bundle.RelationKeyName:                domain.String("High"),
	...
	matches := fx.OptionsNamed("priority", "High")

	require.Len(t, matches, 2)
	assert.Equal(t, []string{"opt-high", "opt-high-2"}, []string{matches[0].Id, matches[1].Id})
}
